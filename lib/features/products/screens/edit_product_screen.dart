...
// ─── Sealed-style image entry ─────────────────────────────────────────────────

class _ImageEntry {
  // Existing image from DB
  final ProductImage? _dbImage;

  // Pending new image
  final XFile? _file;
  final Uint8List? _bytes;

  _ImageEntry._({ProductImage? dbImage, XFile? file, Uint8List? bytes})
      : _dbImage = dbImage,
        _file = file,
        _bytes = bytes;

  factory _ImageEntry.existing(ProductImage img) =>
      _ImageEntry._(dbImage: img);

  factory _ImageEntry.pending(XFile file, Uint8List bytes) =>
      _ImageEntry._(file: file, bytes: bytes);

  bool get isExisting => _dbImage != null;
  bool get isPending => _file != null;

  String? get imageUrl => _dbImage?.imageUrl;
  String? get dbId => _dbImage?.id;
...
class _EditProductScreenState extends ConsumerState<EditProductScreen> {
  ...
  // Campos de ubicación — se leen del producto original, no editables en el form
  String _city = '';
  String _postalCode = '';
  double? _lat;
  double? _lng;
  ...
  void _initFromDetail(ProductDetail detail) {
    ...
    // Sort images by sort_order, then build entries
    final sorted = [...detail.images]
      ..sort((a, b) => a.sortOrder.compareTo(b.sortOrder));

    _entries.clear();
    for (final img in sorted) {
      _entries.add(_ImageEntry.existing(img));
    }

    // Determine cover index
    final coverIdx = _entries.indexWhere((e) => e._dbImage?.isCover == true);
    _coverIndex = coverIdx >= 0 ? coverIdx : 0;
  }
  ...
  Future<void> _pickImage() async {
    ...
    setState(() => _entries.add(_ImageEntry.pending(file, bytes)));
  }
  ...
  void _removeImage(int index) {
    setState(() {
      ...
      if (_entries.isEmpty) {
        _coverIndex = 0;
      } else if (_coverIndex >= _entries.length) {
        _coverIndex = 0;
      }
    });
  }
  ...
  String? _validate() {
    if (_entries.isEmpty) return 'Añade al menos una imagen.';
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // 1. Upload only NEW (pending) images
      final uploadedUrls = <_ImageEntry, String>{};
      for (final entry in _entries) {
        if (entry.isPending) {
          final url = await imageService.uploadImage(entry.file!, user.id);
          uploadedUrls[entry] = url;
        }
      }

      // 2. Determine cover URL
      final coverEntry = _coverIndex < _entries.length
          ? _entries[_coverIndex]
          : _entries.first;
      final coverUrl = coverEntry.isExisting
          ? coverEntry.imageUrl!
          : uploadedUrls[coverEntry]!;
      ...
      // Geocodificar si no hay coordenadas todavía (backfill de productos viejos)
      if (_lat == null || _lng == null) {
        () async {
          await productService.geocodeAndUpdateCoords(
              widget.productId, _city, _postalCode);
        }();
      }

      // 4. Build full image records list (existing + new) and replace
      final imageRecords = <Map<String, dynamic>>[];
      for (int i = 0; i < _entries.length; i++) {
        final entry = _entries[i];
        final url = entry.isExisting
            ? entry.imageUrl!
            : uploadedUrls[entry]!;
        imageRecords.add({
      ...
      await productService.replaceImages(widget.productId, imageRecords);
      ...
      setState(() => _error = 'Error al guardar el producto. Inténtalo de nuevo.');
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return detailAsync.when(
      ...
      data: (detail) {
        ...
        // Pre-fill on first data arrival
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!_initialized) {
            setState(() => _initFromDetail(detail));
          }
        });
            ...
            child: Column(
              ...
              children: [
                ...
                _SectionHeader(
                  ...
                  subtitle: '${_entries.length}/8 imágenes · toca para añadir',
                ),
                const SizedBox(height: 16),
                _PhotoGrid(
                  entries: _entries,
                  coverIndex: _coverIndex,
                  onAdd: _entries.length < 8 ? _pickImage : null,
          ...
          child: Row(
            children: [
              Expanded(
                ...
                child: _ImageSlot(
                  index: 0,
                  entry: entries.isNotEmpty ? entries[0] : null,
                  isCover: coverIndex == 0,
                  onAdd: entries.isEmpty ? onAdd : null,
                  onRemove: () => onRemove(0),
                            ...
                            for (int col = 0; col < 2; col++) ...[
                              ...
                              Expanded(
                                child: () {
                                  ...
                                  return _ImageSlot(
                                    index: i,
                                    entry: entries.length > i ? entries[i] : null,
                                    isCover: coverIndex == i,
                                    onAdd: entries.length == i ? onAdd : null,
                                    onRemove: () => onRemove(i),
...
class _ImageSlot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!hasEntry && index > 0 && onAdd == null) {
      return ClipRRect(
        ...
        child: Container(
          ...
          child: hasEntry
              ? Stack(
                  fit: StackFit.expand,
                  children: [
                    // Render existing images as network, pending as memory
                    if (entry!.isExisting)
                      Image.network(
                        entry!.imageUrl!,
                        fit: BoxFit.cover,
            ...
            children: [
              ...
              Expanded(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const _FieldLabel(label: 'UNIDAD *'),
                    const SizedBox(height: 8),
        ...
        children: [
          ...
          Expanded(
            child: Text(
              message,
              style:
                  GoogleFonts.manrope(fontSize: 13, color: AppTheme.error),
            ),
          ),
        ],
      ),
